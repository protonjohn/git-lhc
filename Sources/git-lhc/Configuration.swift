...
struct Configuration: Codable, Equatable {
        ...
        static let branchRegexes: [Regex<AnyRegexOutput>]? = {
            Configuration.configuration.branchNameLinting?.projectIdRegexes.compactMap {
                do {
                    ...
                    LHC.print("Warning: regex \($0) is invalid: \(error)", error: true)
                    return nil
                }
            }
        }()
    ...
    static var configuration: Self = {
        do {
            ...
            LHC.print("""
                ...
                \(String(describing: LHC.configFilePath)): \(error)
                """,
                error: true
            )
            return .default
    ...
    static func parsed() throws -> Self {
        guard let configFilePath = LHC.configFilePath,
              let contents = LHC.fileManager.contents(atPath: configFilePath) else {
            return .default
        }
...
extension Configuration {
    static let example: Self? = {
        guard let path = Bundle.module.path(forResource: "lhc.example", ofType: "yml") else {
            return nil
        }
