...
extension EnvironmentVariable {
    var value: String? {
        LHC.processInfo.environment[key] ?? defaultValue
    }
}
...
enum LHCEnvironment: String, EnvironmentVariable {
    ...
    var defaultValue: String? {
        ...
        return LHC.fileManager.traverseUpwardsUntilFinding(fileName: ".gluon.yml")
    }
}
...
extension Configuration.Train {
    static var environment: Self? {
        guard let trainName = LHCEnvironment.trainName.value, !trainName.isEmpty else { return nil }
        do {
            return try Configuration.train(named: trainName)
        } catch {
            LHC.print(error, error: true)
            return nil
        }
    }
}
