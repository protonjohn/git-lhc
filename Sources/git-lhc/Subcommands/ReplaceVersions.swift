...
struct ReplaceVersions: ParsableCommand {
    ...
    func replace(in repo: inout Repositoryish, with version: Version) throws {
        ...
        for replacement in replacements {
            ...
            guard var contents = LHC.fileManager.contents(atPath: path) else {
            ...
            try LHC.fileManager.removeItem(atPath: path)
            guard LHC.fileManager.createFile(atPath: path, contents: contents) else {
    ...
    func run() throws {
        ...
        if let forcedVersion {
            ...
            version = Version(0, 0, 1, build: [LHC.timestamp()])
        }
